...
class FFAzureMistralSmall:
    def __init__(self, config: Optional[dict] = None, **kwargs):
        ...
        for key, value in all_config.items():
            match key:
                case 'api_key':
                    self.api_key = value or os.getenv('AZURE_MISTRALSMALL_KEY')
                case 'endpoint':
                    self.endpoint = value or os.getenv('AZURE_MISTRALSMALL_ENDPOINT')
                case 'model':
                    self.model = value
                case 'temperature':
        ...
        self.api_key = getattr(self, 'api_key', os.getenv('AZURE_MISTRALSMALL_KEY'))
        self.endpoint = getattr(self, 'endpoint', os.getenv('AZURE_MISTRALSMALL_ENDPOINT'))
        self.model = getattr(self, 'model', os.getenv('AZURE_MISTRALSMALL_MODEL', defaults['model']))
        self.temperature = getattr(self, 'temperature', float(os.getenv('AZURE_MISTRALSMALL_TEMPERATURE', defaults['temperature'])))
        self.max_tokens = getattr(self, 'max_tokens', int(os.getenv('AZURE_MISTRALSMALL_MAX_TOKENS', defaults['max_tokens'])))
        self.system_instructions = getattr(self, 'system_instructions', os.getenv('AZURE_MISTRALSMALL_ASSISTANT_INSTRUCTIONS', defaults['instructions']))
    ...
    def _initialize_client(self) -> ChatCompletionsClient:
        ...
        logger.info("Initializing Azure MistralSmall client")
    ...
    def generate_response(self, 
        ...
        used_model = model or self.model or "mistral-small-2503"
