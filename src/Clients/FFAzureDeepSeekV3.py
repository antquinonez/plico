...
class FFAzureDeepSeekV3:
    def __init__(self, config: Optional[dict] = None, **kwargs):
        ...
        for key, value in all_config.items():
            match key:
                case 'api_key':
                    self.api_key = value or os.getenv('AZURE_DEEPSEEKV3_KEY')
                case 'endpoint':
                    self.endpoint = value or os.getenv('AZURE_DEEPSEEKV3_ENDPOINT')
                case 'model':
                    self.model = value
                case 'temperature':
        ...
        self.api_key = getattr(self, 'api_key', os.getenv('AZURE_DEEPSEEKV3_KEY'))
        self.endpoint = getattr(self, 'endpoint', os.getenv('AZURE_DEEPSEEKV3_ENDPOINT'))
        self.model = getattr(self, 'model', os.getenv('AZURE_DEEPSEEKV3_MODEL', defaults['model']))
        self.temperature = getattr(self, 'temperature', float(os.getenv('AZURE_DEEPSEEKV3_TEMPERATURE', defaults['temperature'])))
        self.max_tokens = getattr(self, 'max_tokens', int(os.getenv('AZURE_DEEPSEEKV3_MAX_TOKENS', defaults['max_tokens'])))
        self.system_instructions = getattr(self, 'system_instructions', os.getenv('AZURE_DEEPSEEKV3_SYSTEM_INSTRUCTIONS', defaults['instructions']))
    ...
    def generate_response(self, 
        ...
        try:
            ...
            raise RuntimeError(f"Error generating response from Client: {str(e)}")
...
# NOTE: Persistence of conversation history is handled by FFAI wrapper, not here.
