...
class TestClientHistoryIsolation:
    ...
    def test_client_history_cleared_between_prompts(
    ...
    def test_no_double_context(
        self, integration_workbook_with_dependencies, spy_client
    ):
        """
        ...
        assert (
            "step1" in third_prompt or "2 + 2" in third_prompt or "4" in third_prompt
        ), "Third prompt should contain declarative context from step1"
        assert (
            "step2" in third_prompt or "3 + 3" in third_prompt or "6" in third_prompt
    ...
    def test_parallel_clients_dont_share_state(self, integration_workbook, spy_client):
        ...
        orchestrator = ExcelOrchestrator(
            integration_workbook, spy_client, concurrency=3
        )
        orchestrator.run()
    ...
    def test_batch_clients_isolated(
    ...
    def test_clone_preserves_config_but_clears_history(self, real_mistral_client):
        ...
        assert cloned.api_key == real_mistral_client.api_key, (
    ...
    def test_real_api_no_history_accumulation(
        ...
        for r in results:
            ...
            assert r["response"] is not None, (
                f"Prompt {r['sequence']} should have response"
            )

    def test_real_api_parallel_execution_isolated(
        ...
        orchestrator = ExcelOrchestrator(
            integration_workbook, real_mistral_client, concurrency=3
        )
